...
public protocol GoogleAuthServiceInterface {
    ...
    /// The `GoogleProfile` model representing the current `GIDGoogleUser` or `nil` if there is no signed-in user.
    func getCurrentProfile() -> GoogleProfile?
    ...
    func restoreSignIn(handler: @escaping (GoogleAuthServiceRestoreSignInState) -> Void)
    ...
    func signIn(handler: @escaping (Result<GoogleProfile, ACAuthError.GoogleAuthenticationError>) -> Void)
   
    /// Attempts to restore a previously authenticated user without interaction.
    static func restoreSignIn(handler: @escaping (GoogleAuthServiceRestoreSignInState) -> Void)
...
// MARK: - Service
/// An object that implements the authorization methods described in ``GoogleAuthServiceInterface``.
///
...
open class GoogleAuthService: GoogleAuthServiceInterface {
    ...
    public static func restoreSignIn(handler: @escaping (GoogleAuthServiceRestoreSignInState) -> Void)  {
        GIDSignIn.sharedInstance.restorePreviousSignIn(completion: { (user, error) in
            if error != nil || user == nil {
                 handler(.signOut(error: error))
            } else {
                if let user = user {
                    handler(.signIn(profile: GoogleProfile(user: user)))
                } else {
    ...
    public func restoreSignIn(handler: @escaping (GoogleAuthServiceRestoreSignInState) -> Void)  {
        Self.restoreSignIn(handler: handler)
    }

    public func signIn(handler: @escaping (Result<GoogleProfile, ACAuthError.GoogleAuthenticationError>) -> Void) {
        ...
        self.service.signIn(withPresenting: targetView, completion: { (result, error) in
            if let user = result?.user {
                handler(.success(GoogleProfile(user: user)))
            } else {
    ...
    public func getCurrentProfile() -> GoogleProfile? {
        guard let user = service.currentUser else {
            return nil
        }
        return GoogleProfile(user: user)
    }
